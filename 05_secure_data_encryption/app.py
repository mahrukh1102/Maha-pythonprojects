...
if choice == "Home":
    ...
    if st.button("Encrypt and Store"):
        if label and plain_text and passkey:
            ...
            hashed_pass = hash_key(passkey)
            data_store[label] = {"encrypted_text": encrypted, "passkey": hashed_pass}
    ...
    if st.button("Decrypt"):
        if label in data_store:
            correct_hash = data_store[label]["passkey"]
            encrypted_text = data_store[label]["encrypted_text"]

            if hash_key(passkey) == correct_hash:
    ...
    master_pass = st.text_input("Enter Master Password (hint: giaic123)", type="password")

    if st.button("Login"):
        if master_pass == "giaic123":
            st.session_state.failed_attempts = 0
            st.session_state.logged_in = True
